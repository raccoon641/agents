...
class TTS(tts.TTS):
    def __init__(
        ...
        *,
        ...
        # For Gemini TTS models (model_name given), we must not also set a legacy
        # Chirp3 voice name, otherwise the API returns "Voice ... not found" or
        # language mismatch errors. In that case, rely solely on `model_name`.
        if is_given(model_name):
            voice_params.model_name = model_name
        elif is_given(voice_cloning_key):
